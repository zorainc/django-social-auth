...
class OdnoklassnikiLiveTest(LiveServerTestCase):
    ...
    def get_odnoklassniki_name(self):
        raise NotImplementedError(
            'This method is part of interface, but should be implemented '
            'in subclass')
...
class BaseOdnoklassnikiAppTest(OdnoklassnikiLiveTest):
    def setUp(self):
        self.assertTrue(
            hasattr(settings, 'ODNOKLASSNIKI_APP_ID'),
            "You need to have ODNOKLASSNIKI_APP_ID in settings "
            "to test iframe app")
        self.assertTrue(
            hasattr(settings, 'ODNOKLASSNIKI_SANDBOX_DEV_USERNAME'),
            "You need to have ODNOKLASSNIKI_SANDBOX_DEV_USERNAME "
            "in settings to test iframe app")
        self.assertTrue(
            hasattr(settings, 'ODNOKLASSNIKI_SANDBOX_DEV_PASSWORD'),
            "You need to have ODNOKLASSNIKI_SANDBOX_DEV_PASSWORD in "
            "settings to test iframe app")
    ...
    def sandbox_login(self):
        WebDriverWait(self.selenium, 3).until(
            lambda ff: ff.find_element_by_name('j_username'))
    ...
    def sandbox_logout(self):
        url = 'http://api-sandbox.odnoklassniki.ru:8088/sandbox/logout.do'
        self.selenium.get(url)
        WebDriverWait(self.selenium, 3).until(
            lambda ff: ff.find_element_by_name('j_username'))

    def get_odnoklassniki_name(self):
        url = 'http://api-sandbox.odnoklassniki.ru:8088/sandbox/' \
              'protected/main.do'
        self.selenium.get(url)
        self.sandbox_login()
        WebDriverWait(self.selenium, 3).until(
            lambda ff: ff.find_element_by_tag_name('fieldset'))
        self.odnoklassniki_name = self.selenium.find_element_by_xpath(
            '//*[@id="command"]/fieldset/table/tbody/tr[2]/td[2]').text
        self.sandbox_logout()

    def login_into_sandbox(self):
        url = 'http://api-sandbox.odnoklassniki.ru:8088/sandbox/' \
              'protected/application/launch.do?appId={0:s}&userId=0'
        self.selenium.get(url.format(self.app_id))
        self.sandbox_login()
        WebDriverWait(self.selenium, 3).until(
            lambda ff: ff.find_element_by_tag_name('iframe'))
        time.sleep(1)
...
class OdnoklassnikiAppTestExtraData(BaseOdnoklassnikiAppTest):
    @override_settings(ODNOKLASSNIKI_APP_EXTRA_USER_DATA_LIST=('gender',
        'birthday', 'age'))
    def test_extra_data(self):
        ...
        full_name = '{0} {1}'.format(user.first_name, user.last_name)
        self.assertEquals(full_name, self.odnoklassniki_name)
        self.assertTrue(all([field in social_user.extra_data
            for field in ('gender', 'birthday', 'age')]))


class OdnoklassnikiOAuthTest(OdnoklassnikiLiveTest):
    def setUp(self):
        self.assertTrue(hasattr(settings, 'ODNOKLASSNIKI_OAUTH2_CLIENT_KEY'),
                        "You need to have ODNOKLASSNIKI_OAUTH2_CLIENT_KEY "
                        "in settings to test odnoklassniki OAuth")
        self.assertTrue(hasattr(settings, 'ODNOKLASSNIKI_TEST_USERNAME'),
                        "You need to have ODNOKLASSNIKI_TEST_USERNAME in "
                        "settings to test odnoklassniki OAuth")
        self.assertTrue(hasattr(settings, 'ODNOKLASSNIKI_TEST_PASSWORD'),
                        "You need to have ODNOKLASSNIKI_TEST_PASSWORD "
                        "in settings to test odnoklassniki OAuth")
    ...
    def get_odnoklassniki_name(self):
        ...
        WebDriverWait(self.selenium, 3).until(
            lambda ff: ff.find_element_by_id('field_email'))
        ...
        name_css_sel = '#hook_Block_MiddleColumnTopCardUser ' \
                       '.mctc_name>a.mctc_nameLink'
        WebDriverWait(self.selenium, 2).until(
            lambda ff: ff.find_element_by_css_selector(name_css_sel))
        self.odnoklassniki_name = self.selenium.find_element_by_css_selector(
            name_css_sel).text
        #Remember the name of logged user
        link = [el for el in self.selenium.find_elements_by_css_selector(
        ...
        WebDriverWait(self.selenium, 2).until(
            lambda ff: ff.find_element_by_id(
                            'hook_Form_PopLayerLogoffUserForm') and
                       ff.find_element_by_id(
                            'hook_Form_PopLayerLogoffUserForm').is_displayed())
        self.selenium.find_element_by_css_selector(
            '#hook_FormButton_button_logoff').click()
    ...
    def login_into_odnoklassniki(self):
        ...
        WebDriverWait(self.selenium, 2).until(
            lambda ff: ff.find_element_by_name('button_accept_request')
        )
        self.selenium.find_element_by_name('button_accept_request').click()
        self.selenium.implicitly_wait(2)
        time.sleep(1)  # We need this for the server to close database
                       # connection. If this line is removed, following line
                       # will fail
